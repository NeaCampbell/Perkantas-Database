...
CREATE TABLE `logapi` (
  ...
  `dtm_crt` datetime NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=43757 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `member` (
  ...
  `alpha_member_id` bigint DEFAULT NULL,
  `usr_crt` varchar(100) NOT NULL,
  `dtm_crt` datetime NOT NULL,
  `usr_upd` varchar(100) NOT NULL,
  `dtm_upd` datetime NOT NULL,
  ...
  CONSTRAINT `member_ibfk_3` FOREIGN KEY (`city_id`) REFERENCES `city` (`id`)
...
CREATE TABLE `outbox` (
  ...
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `outbox_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCityById`(
	IN city_id_param VARCHAR(100)
)
BEGIN
	DECLARE city_id BIGINT DEFAULT NULL;
	SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
    IF(city_id_param IS NOT NULL AND city_id_param <> "") THEN
		SET city_id = CAST(city_id_param AS UNSIGNED) ;
		SELECT * FROM muridku.`city` WHERE muridku.`city`.`id` = city_id ;
    END IF ;
	SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
BEGIN
    ...
    IF(ktb_id_param IS NOT NULL AND ktb_id_param <> "") THEN
        ...
        ORDER BY hist.`meet_dt` DESC, hist.`material_chapter` DESC ;
    END IF ;
	SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `GetLastKtbMaterialByKtbId` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetLastKtbMaterialByKtbId`(IN ktb_id_param VARCHAR(100))
BEGIN
DECLARE ktb_id BIGINT DEFAULT NULL ;
IF(ktb_id_param IS NULL OR ktb_id_param = "") THEN
SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid ktb id';
END IF ;
SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
SET ktb_id = CAST(ktb_id_param AS UNSIGNED) ;
SELECT material.`id`
, material.`code`
, material.`name`
, ktbhist.`material_name` AS custom_name
, material.`chapter_count`
, ktbhist.`material_chapter` AS chapter
FROM muridku.`material` material
JOIN muridku.`ktbhistory` ktbhist ON ktbhist.material_id = material.id
WHERE ktbhist.ktb_id = ktb_id
ORDER BY ktbhist.meet_dt DESC, ktbhist.material_chapter DESC
LIMIT 0, 1 ;
SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `GetSingleKtbMemberByKtbIdMemberId` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSingleKtbMemberByKtbIdMemberId`(
	IN ktb_id_param VARCHAR(100),
    IN member_id_param VARCHAR(100)
)
BEGIN
    ...
    IF(ktb_id_param IS NOT NULL AND ktb_id_param <> "" AND member_id_param IS NOT NULL AND member_id_param <> "") THEN
		...
		SET member_id = CAST(member_id_param AS UNSIGNED) ;
		SELECT mbr.*
        FROM muridku.`ktbmember` mbr
        WHERE mbr.ktb_id = ktb_id
        AND mbr.member_id = member_id ;
    END IF ;
	SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
BEGIN
	...
	SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
    IF(user_id_param IS NOT NULL AND user_id_param <> "") THEN
		SET user_id = CAST(user_id_param AS UNSIGNED) ;
		SELECT * FROM muridku.`user` WHERE muridku.`user`.`id` = user_id ;
    END IF ;
	SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `GetVersionInfo` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetVersionInfo`(
	IN type_param VARCHAR(100)
)
BEGIN
	SET SESSION TRANSACTION ISOLATION LEVEL READ UNCOMMITTED ;
    IF(type_param IS NOT NULL AND type_param <> "") THEN
		SELECT * FROM muridku.`version` WHERE muridku.`version`.`type` = type_param ;
    END IF ;
	SET SESSION TRANSACTION ISOLATION LEVEL REPEATABLE READ ;
END ;;
DELIMITER ;
...
BEGIN
    ...
    IF(city_id_param IS NULL OR city_id_param = "") THEN
	...
	INSERT INTO muridku.`member` ( `name`, `city_id`, `address`, `usr_crt`, `dtm_crt`, `usr_upd`, `dtm_upd` )
    VALUES ( member_name, city_id, address, usr_ins, dtm_ins, usr_ins, dtm_ins );
...
BEGIN
    ...
    UPDATE muridku.`ktb`
    SET last_meet_dt = meet_dt_param
      , last_material_name = material_name_param
      , last_material_chapter = material_chapter
...
BEGIN
	...
	IF((SELECT COUNT(*) FROM muridku.`city` WHERE LOWER(muridku.`city`.`id`) = city_id LIMIT 0,1) = 0) THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'invalid city id';
	END IF ;
	...
	ELSE
		CALL muridku.`UpdateSingleMember`(`email`, `member_id_param`, `name`, `gender`, `city_id_param`, `address`, birth_dt_param, birth_place, mobile_phn, inst_type, institution_id_param, faculty_id_param, usr) ;
    END IF ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdatePassword`(
	IN user_id_param VARCHAR(100),
    IN pass_param VARCHAR(100)
)
BEGIN
	DECLARE user_id BIGINT DEFAULT NULL;
    
    IF user_id_param = "" THEN
    ...
    UPDATE muridku.`user`
    SET `password` = pass_param
	  , `password_web` = null
    WHERE muridku.`user`.`id` = user_id ;
...
BEGIN
    ...
    SET muridku.`member`.`name` = `name`
      , muridku.`member`.`gender` = gender
      , muridku.`member`.`city_id` = city_id
...
-- Dump completed on 2023-05-18 14:06:56
