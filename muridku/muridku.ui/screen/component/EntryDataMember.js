...
import { EntryDataMemberStyles } from '../../asset/style-template/EntryDataMemberStyles';
import {
...
import Confirmation, { AlertMode, ConfirmMode } from './Confirmation';
...
const EntryDataMember = (props) => {
  ...
  const callbackSaveMember = (result) => {
    ...
    props.dispatch({ type: SET_CURRENT_PAGE, page: props.returnPage });
    setLoading(false);
    navigation.replace(props.returnPage);
  };
  ...
  const onConfirmReturnPage = (value) => {
    if (value) {
      CallbackAction(props, props.childName);
      return;
    }
  ...
  const title = `${isUpdate ? 'Ubah' : 'Tambah'} Data ${isUpdateSelf ? 'Pribadi' : 'AKK'}`;
...
export default connect(mapStateToProps)(EntryDataMember);
