...
CREATE TABLE `logapi` (
  ...
  `dtm_crt` datetime NOT NULL,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=9217 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
BEGIN
	...
	SELECT * FROM muridku.`user` WHERE muridku.`user`.`email` = email AND is_active = 1;
...
BEGIN
    ...
    INSERT INTO muridku.`user` (`member_id`, `email`, `password`, `is_active`, `is_logged_in`, `usr_crt`, `dtm_crt`, `usr_upd`, `dtm_upd`)
    VALUES (member_id, `email`, `password`, 2, 0, usr, dtm_ins, usr, dtm_ins);
...
BEGIN
	...
	IF((SELECT COUNT(*) FROM muridku.`user` WHERE muridku.`user`.`member_id` = `member_id` AND muridku.`user`.`is_active` = 1 LIMIT 0,1) = 1) THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'email already activated';
	END IF ;
...
-- Dump completed on 2021-05-27 18:11:06
